...
private:
				   ...
				   vector<int>& p_candidateMatched, vector<int>& p_currentMatched, bool& p_match)
	{
		...
		for (int i = 0; i < p_candidateMatched.size() && !p_match; ++i)
		{
			bool m_matched = false;
			m_candidateIndex = p_candidateMatched[i];

			for (int j = 0; j < p_primaryMatched.size() && !p_match; ++j)
			{
...
public:
	...
	//---------------------------------------------------------------------------
	inline int			Size() const { return _adjacencyMatrix.size(); }
	//---------------------------------------------------------------------------
	...
	//---------------------------------------------------------------------------
	int AddNode(TValue p_value, int p_index)
	{
		for (int i = 0; i < _adjacencyMatrix.size(); ++i)
		{
		...
		for (int i = 0; i < m_roots.size(); ++i)
		{
			for (int j = 0; j < _adjacencyMatrix.size(); ++j)
			{
				if (_adjacencyMatrix[j]->Equals(*p_candidate[m_roots[i]]))
				{
					...
					if(m_match)
					{
						for (int k = 0; k < m_currentMatched.size(); ++k)
						{
